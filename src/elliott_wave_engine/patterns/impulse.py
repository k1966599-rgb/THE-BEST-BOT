...
def wave_3_extension_rule(points: List[WavePoint]) -> WaveRuleResult:
    ...
    return WaveRuleResult("إرشاد: امتداد الموجة 3", passed, f"W3:{len3:.2f} > W1,W5")

def wave_5_divergence_rule(points: List[WavePoint], data: pd.DataFrame) -> WaveRuleResult:
    if 'rsi' not in data.columns: return WaveRuleResult("إرشاد: دايفرجنس الموجة 5", False, "لم يتم العثور على مؤشر RSI")
    ...
    return WaveRuleResult("إرشاد: دايفرجنس الموجة 5", passed, f"Price:{'HH' if is_bullish else 'LL'}, RSI:{'LH' if rsi_p5 < rsi_p3 else 'HL'}")
...
def wave_5_truncation_rule(points: List[WavePoint]) -> WaveRuleResult:
    ...
    return WaveRuleResult("إرشاد: انقطاع الموجة 5", passed, f"P5({p5.price:.2f}) vs P3({p3.price:.2f})")
...
def rule_of_alternation(points: List[WavePoint]) -> WaveRuleResult:
    ...
    return WaveRuleResult("إرشاد: التبادل", passed, f"W2 retrace:{ratio2:.1%}, W4 retrace:{ratio4:.1%}")
...
def wave_2_4_time_similarity_rule(points: List[WavePoint]) -> WaveRuleResult:
    ...
    if dur2 == 0 or dur4 == 0: return WaveRuleResult("إرشاد: تشابه زمني", False, "Zero duration wave")
    ratio = min(dur2, dur4) / max(dur2, dur4)
    return WaveRuleResult("إرشاد: تشابه زمني", passed=ratio > 0.618, details=f"W2:{dur2} candles, W4:{dur4} candles")
